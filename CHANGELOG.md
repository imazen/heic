...
- **`zencodec` bumped to the released `0.1.26`** (`Cargo.toml`), which ships the two-level origin-first `ErrorCategory` reshape (PR #116) this feature adopts. The unreleased-branch `[patch.crates-io]` git pin is gone, but a `[patch.crates-io]` entry for `zencodec` remains, now pinned to the `v0.1.26` tag: `zencodec-testkit` (dev-dependency, an unpublished-by-design conformance-harness workspace member of the zencodec repo) depends on its sibling `zencodec` crate via `path = ".."`, not the registry, so without a patch unifying both onto the same git source, cargo resolves two distinct `zencodec` crate instances (registry + git-path-copy) and every testkit conformance check fails to typecheck (`E0277`, "multiple different versions of crate `zencodec`"). The tag is content-identical to the crates.io release, so this is a graph-unification patch, not a version skew.

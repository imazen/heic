...
### Fixed
- **HEVC decoder: 4:2:2 streams (`chroma_format_idc = 2`) are now rejected up front with `HevcError::Unsupported` instead of being mis-decoded** (`src/hevc/mod.rs`, #48). The CTU/transform path only distinguishes 4:4:4 from "everything else", so 4:2:2 fell through the 4:2:0 branch: one `cbf_cb`/`cbf_cr` read per depth where H.265 7.3.8.8 codes two for `ChromaArrayType == 2`, chroma TBs placed at `y0 / 2` instead of `y0`, no Table 8-3 chroma-mode remap. The skipped bins desync CABAC, so 8-bit 4:2:2 failed with a misleading "truncated bitstream" / `cu_qp_delta` error a few CTBs in, and 10-bit 4:2:2 returned `Ok` with chroma samples ~17x outside the legal range. Both now fail fast (still images and `VideoDecoder`). The `87c79fd` monochrome entry's "4:2:0/4:2:2/4:4:4 unaffected" claim was wrong for 4:2:2 — it is listed under Known limitations in the README now. New fixtures (`scripts/gen_422_fixtures.py`: x265 `--lossless` raw streams with their i422 sources, heif-enc `chroma=422` HEICs with dec265 reference YUV) pin the behaviour in `tests/chroma422.rs`.

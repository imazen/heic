...
- Diagnostic harness behind `HEIC_D3D11VA_DEBUG=1`: prints SPS+PPS flag combinations, NAL framing, and first-pixel samples per decode call. Used to triage the remaining 2 corpus failures (example.heic 1280×854 grid + apple-hdr/hdr-sample 10-bit P010) — both produce midgray output, sdh=false in PPS, likely scaling-list-data or P010 pic-param-layout root cause. See "Known Bugs" in `CLAUDE.md` for the triage notes.

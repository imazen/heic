...
- **`zencodec` bumped to the released `0.1.26`** (`Cargo.toml`), which ships the two-level origin-first `ErrorCategory` reshape (PR #116) this feature adopts; the `[patch.crates-io]` git pin used while 0.1.26 was unreleased is dropped. `zencodec-testkit` (dev-dependency) stays a git dependency — it is an unpublished-by-design conformance-harness workspace member of the zencodec repo, not a crate that will ever ship to crates.io — now pinned to the `v0.1.26` release commit for reproducibility.

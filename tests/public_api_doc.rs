...
//!
//! Adaptation for this workspace: the parent `heic` crate has an
//! intentionally-empty default feature set (building with no backend feature
//! hits the `compile_error!` gate in src/lib.rs), so its baseline section is
//! "default features + backend-rust" — the smallest buildable surface.
//!
//! The native-backend crates are platform-gated, so their snapshots reflect
//! the HOST rustdoc view. Linux is canonical (CI verifies on ubuntu); if you
//! regen on macOS/Windows and a backend snapshot churns, don't commit it.
//!
...
/// Published crates in this workspace, in RELEASING.md publish order.
const CRATES: &[&str] = &[
    "heic-core",
...
/// Features required for the crate's baseline rustdoc build to succeed at
/// all. The parent crate `compile_error!`s without at least one backend
/// feature; `backend-rust` is the platform-independent one.
fn baseline_features(package: &str) -> &'static [&'static str] {
    match package {
        "heic" => &["backend-rust"],
        _ => &[],
    }
...
fn public_api_surface_docs_are_current() {
    ...
    for package in CRATES {
        let baseline = baseline_features(package);
        let baseline_csv = baseline.join(",");
        let baseline_label = if baseline.is_empty() {
            "default features".to_string()
        } else {
            format!("default features + {baseline_csv}")
        };
        let features = public_features(package);
        let feature_csv = features.join(",");
        let sections: [(String, Vec<&str>); 2] = [
            (
                baseline_label,
                if baseline.is_empty() {
                    vec![]
                } else {
                    vec!["--features", &baseline_csv]
                },
            ),
            (
                "all features except _*".to_string(),

...
fn compare_display_order() {
    ...
    eprintln!("=== Display-order comparison (girlshy, cropped {}x{}) ===", w, h);
        ...
        for row in 0..4u32 {
            ...
            for y in y_start..y_end {
                for x in 0..w {
                    ...
                    if idx < len {
                        row_total += 1;
                        if our_y[idx] == ref_y[idx] { row_exact += 1; }
                    }
                }
            }
            let pct = if row_total > 0 { 100.0 * row_exact as f64 / row_total as f64 } else { 0.0 };
            row_info.push_str(&format!(" R{row}:{pct:.0}%"));
        }

        eprintln!("  Frame {fi:2}: PSNR={psnr:6.1}dB exact={exact}/{len} ({:4.1}%) max={max_diff:3}{row_info}",
